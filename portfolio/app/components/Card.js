...
const Card = (props) => {
  ...
  return (
    <div
      className='cursor-pointer z-30 relative font-sans font-medium '
    ...
    >
      ...
      <div className='relative overflow-hidden rounded-lg group'>
        <Image
          className='w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-300 rounded-sm shadow-lg backdrop-blur-sm '
          src={props.image}
          width={props.width}
          height={props.height}
          alt={props.alt}
        ...
        <div className='absolute inset-0 bg-[#1E1E1E] opacity-0 group-hover:opacity-70 transition-opacity rounded-lg' ></div>
        ...
        {props.liveLink && (
          <div className='hidden group-hover:flex absolute inset-0 justify-center items-center'>
            <a
              ...
              className='px-3 py-2 bg-[#E6E6E6] text-[#1A1A1A] text-xs rounded-md shadow-lg flex items-center gap-2 hover:bg-[#1A1A1A] hover:text-white'
        ...
        <div className='absolute top-3 left-3 right-3 flex justify-between items-center p-2'>
          <div className='flex gap-2'>
            {props.tags.map((tag, index) => (
              <span key={index} className='block md:hidden lg:block bg-[#E6E6E6] text-[#1A1A1A] px-2 py-[2px] text-[10px] rounded-md group-hover:bg-[#1A1A1A] group-hover:text-white group-hover:px-2 group-hover:py-[3px] group-hover:shadow-lg'>
                {tag}
              </span>
            ))}
          </div>
          <span className='bg-[#E6E6E6] text-[#1A1A1A] text-sm font-semibold rounded-full w-5 h-5 flex justify-center items-center'>
        ...
        <div className='hidden group-hover:flex absolute bottom-3 left-3 right-3 flex-col gap-2 p-2'>
          <div className='flex gap-2'>
            <span className='px-2 py-[2px] text-[10px] rounded-md bg-[#1A1A1A] text-white group-hover:shadow-lg'>
              Role
            </span>
            {props.role.map((item, index) => (
              <span key={index} className=' bg-[#E6E6E6] text-[#1A1A1A] px-2 py-[2px] text-[10px] rounded-md group-hover:shadow-lg'>
                {item}
              </span>
            ))}
          </div>
      ...
      <div className='pt-2 pb-8 flex flex-col gap-2'>
        <div className=' relative z-30  select-none font-mono uppercase font-thin text-sm text-gray-100 tracking-tighter'>{props.title}</div>
        {/* <div className=''>
          {props.description}
        </div> */}
      </div>
...
export const MouseTooltip = () => {
  ...
  return (
    <div
      className="fixed flex gap-2 items-center mt-6 px-3 py-1 bg-[#FF00C1] text-[#121212] font-semibold  rounded-e-xl rounded-b-xl text-xs opacity-90 whitespace-nowrap pointer-events-none z-50"
      style={{
